...
/** Section frame for the case study, on the same rail as every other section. */
function Block({
  label,
  children,
  soft = false,
      ...
      <main>
        <header className="wash-top">
          <div className="mx-auto max-w-5xl px-6 pb-12 pt-12 sm:px-10 sm:pb-20 sm:pt-16">
            <Link
              href="/#work"
              className="link-quiet inline-flex items-center gap-2 text-note text-muted"
            >
            ...
            <div className="rise mt-8">
              ...
              <h1 className="t-name mt-4 max-w-[18ch] text-ink">
                {project.name}
              </h1>

              <p className="mt-6 max-w-[52ch] font-medium text-ink-2">
                {project.tagline}
              </p>

              <MetaLine
                className="mt-6 text-note text-muted"
                items={[
                  project.role,
                  <span key="when" className="t-meta">{project.period}</span>,
                  project.access,
                ]}
              />
            </div>
        ...
        <Block label="The problem">
          <Reveal>
            <div className="space-y-6">
              {study.problem.map((p) => (
                <p key={p.slice(0, 24)} className="max-w-[70ch] text-prose text-body">
                  {p}
                </p>
              ))}
            </div>
        ...
        <Block label="My part" soft>
          <Reveal>
            <p className="max-w-[66ch] text-prose text-body">
              {study.responsibility}
            </p>
            <p className="mt-6 max-w-[66ch] text-prose text-body">
              The parts I was responsible for: {lower(project.owned).join(", ")}.
            </p>
          </Reveal>
          <Reveal delay={80}>
            <h2 className="t-label mt-8 text-faint">Built with</h2>
            <ul className="mt-4 flex flex-wrap items-center gap-x-6 gap-y-3">
              {project.stack.map((t) => (
                <li key={t.label} className="inline-flex items-center gap-2 text-fine text-body">
        ...
        {study.surfaces ? (
          <Block label="The surfaces">
            <Reveal>
              <p className="max-w-[64ch] text-prose text-body">{project.hardPart}</p>
            </Reveal>
              ...
              {study.surfaces.map((s, i) => (
                <Reveal key={s.name} delay={i * 50}>
                  <li className="border-t border-line pt-6">
                    <h3 className="t-h3 text-lead text-ink">{s.name}</h3>
                    <p className="mt-3 text-note text-body">{s.body}</p>
                  </li>
                </Reveal>
              ))}
            </ul>
        ...
        <section id="decisions" className="wash-dark scroll-mt-20">
          <div className="mx-auto max-w-5xl px-6 sm:px-10">
            <Rail label="Decisions" tone="dark">
              <div>
                <Reveal>
                  <h2 className="t-h2 max-w-[24ch] text-white">
                    ...
                    <Reveal key={d.title} delay={i * 60}>
                      <li className={i > 0 ? "mt-12 pt-12" : ""}>
                        ...
                        <div className="flex items-baseline gap-4">
                          ...
                          <h3 className="t-h3 max-w-[34ch] text-white">
                            {d.title}
                          </h3>
                        </div>
                        ...
                        <dl className="mt-6 sm:pl-[38px]">
                          {DECISION_ROWS.map((row) => (
                            ...
                            >
                              ...
                              <dd className="max-w-[64ch] text-note text-[#a8c0cf]">
                                {d[row.key]}
                              </dd>
                            </div>
                          ))}
          ...
          <ul className="space-y-4">
            {study.outcome.map((o, i) => (
              <Reveal key={o} delay={i * 50}>
                <li className="relative max-w-[70ch] pl-6 text-prose text-body">
        ...
        {study.notBuilt ? (
          <Block label="Not built" soft>
            <Reveal>
              <p className="max-w-[60ch] text-prose text-body">
            ...
            <ul className="mt-8 space-y-6">
              {study.notBuilt.map((n, i) => (
                <Reveal key={n} delay={i * 50}>
                  <li className="relative max-w-[68ch] pl-6 text-note text-body">
                    <span className="absolute left-0 top-[12px] h-px w-3 grad-hairline" />
                    {n}
                  </li>
                </Reveal>
          ...
          <div className="mx-auto max-w-5xl px-6 sm:px-10">
            ...
            <div className="flex flex-col gap-8 py-16 sm:flex-row sm:items-end sm:justify-between sm:py-20">
              <div>
                ...
                {others.map((o) => (
                  <Link
                    key={o.slug}
                    href={`/work/${o.slug}`}
                    className="t-h3 link mt-3 block max-w-[16ch] text-ink"
                  >
                    {o.name}
                  </Link>
                ))}
              ...
              <a
                ...
                className="link flex-none self-start text-note font-medium text-link sm:self-auto"
              >
